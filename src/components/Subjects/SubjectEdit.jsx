...
const SubjectEdit = ({ subject, onClose, onSubjectUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const updatedSubject = await updateSubject(subject._id, formData);
      setMessage("Thông tin môn học đã được cập nhật thành công!");
      onSubjectUpdated(updatedSubject);
      onClose();
    } catch (error) {
      console.error("Error updating subject:", error.response?.data || error.message);
      setMessage("Có lỗi xảy ra khi cập nhật thông tin môn học.");
    }
  };
